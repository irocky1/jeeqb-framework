...
import org.jeeqb.framework.mvc.Handler;
import org.jeeqb.framework.mvc.HandlerInvoker;
import org.jeeqb.framework.mvc.UploadHelper;
...
public class DefaultHandlerInvoker implements HandlerInvoker {
    ...
    private ViewResolver viewResolver = InstanceContext.getViewResolver();
    ...
    public void invoke(HttpServletRequest request, HttpServletResponse response, Handler handler) throws Exception {
        ...
        // 解析视图
        viewResolver.resolve(request, response, actionMethodResult);
    }
    ...
    public List<Object> createActionMethodParamList(HttpServletRequest request, Handler handler) throws Exception {
        ...
        paramList.addAll(createPathParamList(handler.getPathMatcher(), actionParamTypes));
        // 分两种情况进行处理
        if (UploadHelper.isMultipart(request)) {
            ...
            if (MapUtil.isNotEmpty(requestParamMap)) {
                paramList.add(new RequestParams(requestParamMap));
            }
        }
        // 返回参数列表
        return paramList;
