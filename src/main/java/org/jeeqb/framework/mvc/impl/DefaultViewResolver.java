...
import org.jeeqb.framework.mvc.bean.ResponseResult;
import org.jeeqb.framework.mvc.bean.View;
...
public class DefaultViewResolver implements ViewResolver {
    ...
    @Override
    public void resolve(HttpServletRequest request, HttpServletResponse response, Object actionResult) {
        if (actionResult != null) {
            // Action 返回值可为 View 或 Result
            if (actionResult instanceof View) {
                // 若为 View，则需考虑两种视图类型（重定向 或 转发）
                View view = (View) actionResult;
                if (view.isRedirect()) {
                    // 获取路径
                    ...
                    // 获取路径
                    String path = JEEQB.JSP_PATH + view.getPath();
                    // 初始化请求属性
                ...
                ResponseResult result = (ResponseResult) actionResult;
